...
////////////////////////////////////////////////////////////////////////////////
void ExternalPotential::update(const ChargeDensity& cd, const Context& new_col_ctxt)  
{
  const Context* ctxt = s_.wf.spincontext(0);
  bool onpe0 = ctxt->onpe0();
  int nprow = ctxt->nprow();
  int myrow = ctxt->myrow();
  int mycol = ctxt->mycol();
  MPI_Comm vcomm = MPI_COMM_WORLD; 
  ...
  int n1max = (n_[1]-2)/2;
  int n2max = (n_[2]-2)/2;
  double ecut0 = 0.5 * norm(cell.b(0)) * n0max*n0max;
  double ecut1 = 0.5 * norm(cell.b(1)) * n1max*n1max;
  double ecut2 = 0.5 * norm(cell.b(2)) * n2max*n2max;
  ecut_ = min(min(ecut0,ecut1),ecut2);
  Basis basis(new_col_ctxt, D3vector(0,0,0), true);
...
double ExternalPotential::compute_eext(const ChargeDensity& cd)
{
  ...
  double eext_sum = 0.0;
  MPI_Allreduce(&eext,&eext_sum,1,MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD);
